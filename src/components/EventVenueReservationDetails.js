...
import { useEventVenueReservationsContext } from "../hooks/useEventVenueReservationsContext.js";
...
const EventVenueReservationDetails = ({ eventVenueReservation }) => {
  ...
  const { setEventVenueReservations } = useEventVenueReservationsContext();
  ...
  const handleDelete = async () => {
    ...
    if (response.ok) {
      setEventVenueReservations({ type: "DELETE_eventVenueRESERVATION", payload: json });
    }
  };
    ...
    <>
      ...
      >
        ...
        <td className="px-6 py-4">
          {eventVenueReservation.guest.title} {eventVenueReservation.guest.firstName}{" "}
          ...
          <tr className=" bg-gray-50 font-medium text-gray-600 whitespace-nowrap ">
            ...
            <td className="pt-4 pb-2" colSpan="7">
              {eventVenueReservation.eventVenues.map((eventVenue, index) => (
                <p key={index}>
                  {eventVenue.eventVenueNo} - {eventVenue.type}
                </p>
              ))}
            </td>
          </tr>
