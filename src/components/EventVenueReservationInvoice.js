...
const EventVenueReservationInvoice = ({
  ...
  const generatePdf = () => {
    ...
    logo.onload = () => {
      ...
      const body = [
        [
          { content: "Venue/s", styles: { fontStyle: "bold", halign: "left" } },
      ...
      if (packageType) {
        body.push(
          ...
          [`${packageType.name} `, `${packageType.cost} per guest`],
          [`No of guests`, `${guestCount}`],
          [`Total Package cost`, `${guestCount * packageType.cost}`],
          [("", "")]
        );
      }
      ...
      if (reservedRooms && reservedRooms.length > 0) {
        body.push(
          [
            { content: "Room", styles: { fontStyle: "bold", halign: "left" } },
            { content: "", styles: { halign: "center" } },
          ],
